...
describe("IdentityService", () => {
		it("delegates all methods to the platform client", async () => {
				...
				const service = new IdentityService(fakeClient as never);
				...
				const contextResult = await service.getIdentityContext();
				expect(contextResult).toEqual(fakeResponse);
				expect(fakeClient.getIdentityContext).toHaveBeenCalledTimes(1);
				expect(fakeClient.getIdentityContext).toHaveBeenCalledWith(undefined);
